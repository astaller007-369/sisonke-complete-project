...
def fetch_thestatsapi_to_sisonke(league_id, target_seasons):
    ...
    # ðŸ›‘ PASTE YOUR REAL APINUMERIC TOKEN STRINGS INSIDE THESE QUOTES:
    ...
    for season in target_seasons:
        ...
        # ðŸŸ¢ THE REAL FIXED PATH: Appends the /api route and maps clean ID strings
        ...
        try:
            # ðŸŸ¢ FIXED: Hardcodes your token directly to bypass the missing variable fault
    ...
    # ðŸ›¡ï¸ STAGE 3: APPLY SURGICAL DATA FILTERING LOCKS
...
if active_app_tab == "📁   Research & Sentiment Tracker":
    ...
    league_col_target = None
    
    # OFFLINE STORAGE HOOK: Reads your permanent drive data directly
    if os.path.exists(database_csv_path):
        try:
            ...
            # Dynamically identify whichever column variant your file is using
            if "league_country" in master_db_df.columns:
                league_col_target = "league_country"
            elif "league" in master_db_df.columns:
                league_col_target = "league"
            elif "competition" in master_db_df.columns:
                league_col_target = "competition"
                
            if league_col_target:
                available_leagues = sorted(master_db_df[league_col_target].dropna().unique().tolist())
        except Exception:
            pass
    ...
    if not master_db_df.empty and league_col_target is not None:
        # Filter rows belonging to the active league selection block
        league_mask = master_db_df[league_col_target] == selected_league
        filtered_league_df = master_db_df[league_mask]
        ...
        if not filtered_league_df.empty and "home" in filtered_league_df.columns and "away" in filtered_league_df.columns:
            for _, row in filtered_league_df.iterrows():
                ...
                is_unplayed_by_comma = "," in home_goals_raw or "," in away_goals_raw
                is_unplayed_by_nan = pd.isna(row.get("home_goals")) or pd.isna(row.get("away_goals"))
                
                if is_unplayed_by_comma or is_unplayed_by_nan:
    ...
    if match_id_key not in persisted_data:
        persisted_data[match_id_key] = {
            ...
            "motivation": "Mid-table, Late Season (ðŸ˜´ Beach Mode / High Underperformance Risk)",
            "notes": "",
    ...
    if not master_db_df.empty and "home" in master_db_df.columns:
            # ðŸŸ¢ CRASH FIX: Move the team splitter logic BEFORE the try statement block
        ...
        try:
            ...
            if has_odds_columns and current_home_team and current_away_team:
                # ðŸ  Step 1: Calculate average price movements for the Home Team
                ...
                # âœˆï¸ Step 2: Calculate average price movements for the Away Team
    ...
    with tracking_col:
        ...
        def check_market_shift(open_odds, current_odds):
            ...
            if delta_pct > 1.5: return f"ðŸ”¥ STEAM: Dropped by {abs(delta_pct):.1f}% (Sharp Inflow)"
            elif delta_pct < -1.5: return f"âš ï¸ DRIFT: Rose by {abs(delta_pct):.1f}% (Public Fade)"
            return "Stable Line"
        ...
        with st.expander("📉🏆Bookmaker Outrights Radar (League Winner / Relegation)"):
            ...
            # ==============================================================================
            # ðŸŸ¢ CHANGE 'format_shift_string' TO 'check_market_shift' ON THIS ROW:
            st.info(f"**Outright Market Trend:** {check_market_shift(out_open, out_close)}")

        
        st.markdown("---")
        st.header("ðŸ§  Tactical Environmental Modifiers")
        match_state["motivation"] = st.selectbox("Fixture Motivation Level Profile:", [
            "Mid-table, Late Season (ðŸ˜´ Beach Mode / High Underperformance Risk)",
            "Relegation Battle (ðŸ”¥ Maximum Effort / Underdog Defense Boost)",
            "Champions League Qualification / Promotion Race (âš¡ High Box Intensity)",
            "Derby Matches (ðŸ“ˆ Pride Match / High Variance / Form Breaks)"
        ], index=0, key="sb_mot_nv")

                # ==============================================================================
        # AUTOMATED CHECKBOX CALCULATOR LOCK
        # ==============================================================================
        # ðŸŸ¢ PLACE THIS SINGLE LINE DIRECTLY ABOVE YOUR base_score EQUATION:
        checked_count = sum(1 for v in match_state["checklist"].values() if v is True)
    
        # ðŸ§® STEP 2: ALGORITHMIC CONFIDENCE AUTOMATION
...
league_directory = {
    ...
    "Brazil SÃ©rie A": 1262,
    "Brazil SÃ©rie B": 1263, 
...
} # ðŸ›‘ BRACKET IS CLOSED FLUSH FORWARD WITH ZERO TRAILING COMMAS!
...
# ðŸ“ THE NEW SEASON INPUT TEXT BOX
...
with st.expander("🛠️🧮 Advanced Calibration & Mathematical Tuning Vault", expanded=False):
    ...
    if activate_manual_decay_override:
        ...
        rho_parameter_input = st.slider("Manual Dixon-Coles Rho (Ï) Adjustment", -0.25, 0.25, float(automatically_tuned_rho_parameter), 0.01)
    else:
    ...
    trigger_wipe_database_execution = clear_c2.button("ðŸš¨ WIPE ALL DATABASE RECORDS", key="btn_wipe_db_core_vault")
...
# ==============================================================================
            ...
            raw_matrix_dictionary_build = [
                ...
                # ==============================================================================
        # DUAL-ENGINE INTERACTIVE EXPANSION BLOCK (PASTED DIRECTLY AT THE TAIL OF SEGMENT 10)
        # ==============================================================================
        # Step 1: Run a 10,000-Iteration local Monte Carlo simulation loop in memory
        mc_runs_pass = 10000
        
        # Pull baseline goals from your active variables to build a matched random distribution
        ...
        # Step 2: Compile your new side-by-side analytics payload grid records
        market_payload_records = []
        
        for market_label, market_odds, dc_math_probability, trajectory_tier in raw_matrix_dictionary_build:
            # Dynamically calculate matching Monte Carlo probabilities based on the mock runs
            if "HOME WIN" in market_label: mc_prob_val = float(np.sum(simulated_h_goals > simulated_a_goals) / mc_runs_pass)
            elif "DRAW MATCH" in market_label: mc_prob_val = float(np.sum(simulated_h_goals == simulated_a_goals) / mc_runs_pass)
            elif "AWAY WIN" in market_label: mc_prob_val = float(np.sum(simulated_h_goals < simulated_a_goals) / mc_runs_pass)
            elif "OVER 2.5" in market_label: mc_prob_val = float(np.sum((simulated_h_goals + simulated_a_goals) > 2.5) / mc_runs_pass)
            elif "UNDER 2.5" in market_label: mc_prob_val = float(np.sum((simulated_h_goals + simulated_a_goals) <= 2.5) / mc_runs_pass)
            elif "BOTH TEAMS TO SCORE (YES)" in market_label: mc_prob_val = float(np.sum((simulated_h_goals > 0) & (simulated_a_goals > 0)) / mc_runs_pass)
            elif "BOTH TEAMS TO SCORE (NO)" in market_label: mc_prob_val = float(np.sum((simulated_h_goals == 0) | (simulated_a_goals == 0)) / mc_runs_pass)
            elif "DRAW NO BET (DNB1)" in market_label:
            ...
            elif "DRAW NO BET (DNB2)" in market_label:
            ...
            elif "HOME TOTAL GOALS OVER 1.5" in market_label: mc_prob_val = float(np.sum(simulated_h_goals > 1.5) / mc_runs_pass)
            elif "HOME TOTAL GOALS UNDER 1.5" in market_label: mc_prob_val = float(np.sum(simulated_h_goals <= 1.5) / mc_runs_pass)
            elif "AWAY TOTAL GOALS OVER 1.5" in market_label: mc_prob_val = float(np.sum(simulated_a_goals > 1.5) / mc_runs_pass)
            elif "AWAY TOTAL GOALS UNDER 1.5" in market_label: mc_prob_val = float(np.sum(simulated_a_goals <= 1.5) / mc_runs_pass)
            elif "HOME CLEAN SHEET (YES)" in market_label: mc_prob_val = float(np.sum(simulated_a_goals == 0) / mc_runs_pass)
            elif "AWAY CLEAN SHEET (YES)" in market_label: mc_prob_val = float(np.sum(simulated_h_goals == 0) / mc_runs_pass)
            elif "DOUBLE CHANCE (1X)" in market_label: mc_prob_val = float(np.sum(simulated_h_goals >= simulated_a_goals) / mc_runs_pass)
            elif "DOUBLE CHANCE (X2)" in market_label: mc_prob_val = float(np.sum(simulated_h_goals <= simulated_a_goals) / mc_runs_pass)
            elif "DOUBLE CHANCE (12)" in market_label: mc_prob_val = float(np.sum(simulated_h_goals != simulated_a_goals) / mc_runs_pass)
            else: mc_prob_val = dc_math_probability # Safe fallback for handicaps
            
            #  Compute your advanced new column features natively
            ...
            market_payload_records.append({
                "Derivative Trading Market": market_label,
                "Dixon-Coles Prob": f"{dc_math_probability * 100:.1f}%",
                "Monte Carlo Prob": f"{mc_prob_val * 100:.1f}%",
                "Convergence %": f"{market_convergence_score:.1f}%",  #  CONVERGENCE COLUMN
...
# SEGMENT 12 OF 14: GRAPH CABINET EXPANDERS & FPE-EQUIPPED VALUATION SHEET
 # ==============================================================================
            ...
            st.dataframe(pd.DataFrame(all_markets_rendered_rows), use_container_width=True, hide_index=True)
                        ...
                        # ==============================================================================
        # ADDED TO SEGMENT 12: INTERACTIVE SISONKE MULTI-LEG PARLAY & KELLY CALCULATOR
        # ==============================================================================
        st.markdown("---")
        st.header("🎟️ Sisonke Multi-Leg Parlay Slip Builder")
        st.markdown("Select qualifying value legs from your matrix sheet to calculate combined parlay efficiency and Kelly bankroll staking fraction.")

        # Guard check: Ensure we have active markets to choose from
        if 'all_markets_rendered_rows' in locals() and all_markets_rendered_rows:
            
            # Create a clean lookup list of your active market predictions
            parlay_options_list = [
                f"{row['Betting Market']} @ {row['Bookmaker Odds']} (Prob: {row['Dixon-Coles Prob']})" 
                for row in all_markets_rendered_rows
            ]
            
            selected_parlay_legs = st.multiselect(
                "Build Your Parlay Slip (Select 2 or more lines):",
                options=parlay_options_list,
                key="sisonke_parlay_multiselect_v2026_upgraded"
            )
            
            if len(selected_parlay_legs) >= 2:
                combined_bookmaker_odds = 1.0
                combined_model_probability = 1.0
                parlay_market_names = []
                
                # Match selected string tokens back to their raw numeric variables
                for selected_text in selected_parlay_legs:
                    for row in all_markets_rendered_rows:
                        # Extract matching market rows safely
                        if row['Betting Market'] in selected_text:
                            # Strip out string formatting to pull numbers behind the glass
                            raw_odds_val = float(row['Bookmaker Odds'])
                            raw_prob_val = float(row['Dixon-Coles Prob'].replace('%', '')) / 100.0
                            
                            combined_bookmaker_odds *= raw_odds_val
                            combined_model_probability *= raw_prob_val
                            parlay_market_names.append(f"- {row['Betting Market']} @ {raw_odds_val:.2f}")
                            break
                
                # Compute your true multi-leg parlay edge profiles
                combined_fair_odds = 1.0 / max(0.001, combined_model_probability)
                combined_parlay_ev = (combined_model_probability * combined_bookmaker_odds) - 1.0
                
                # Render the visual statistical breakdown cards on your layout view
                p_col1, p_col2, p_col3 = st.columns(3)
                with p_col1:
                    st.metric(
                        label="Combined Sportsbook Odds", 
                        value=f"{combined_bookmaker_odds:.2f}x"
                    )
                with p_col2:
                    st.metric(
                        label="True Model Fair Odds", 
                        value=f"{combined_fair_odds:.2f}x"
                    )
                with p_col3:
                    st.metric(
                        label="Combined Multi-Leg Edge", 
                        value=f"{combined_parlay_ev * 100:+.1f}%",
                        delta="🔥 ELITE PARLAY" if combined_parlay_ev >= 0.05 else "⚠️ COMPREHENSIVE TRAP"
                    )
                
                # 🧮 STEP 2: INTEGRATED KELLY CRITERION FRACTION ALGORITHM
                st.markdown("##### 🎚️ Fractional Kelly Criterion Risk Settings")
                
                # Dynamic link to user campaign bankroll pool input from Segment 11
                active_pool = float(user_matchday_bankroll_pool) if 'user_matchday_bankroll_pool' in locals() else 5000.0
                
                # Kelly Slider scale to keep users from over-betting bankroll variables
                kelly_fraction_scalar = st.slider(
                    "Kelly Multiplier Scale Fraction (Quarter-Kelly Recommended):", 
                    0.05, 1.00, 0.25, 0.05,
                    help="Decreasing your fraction lowers volatility and protects your bankroll from bad luck streaks."
                )
                
                # Pure Kelly Equation: f* = (bp - q) / b = (Odds-1 * Prob - (1-Prob)) / Odds-1
                # Which simplifies beautifully down to: EV / (Odds - 1)
                b_decimal_odds_minus_one = combined_bookmaker_odds - 1.0
                
                if b_decimal_odds_minus_one > 0 and combined_parlay_ev > 0:
                    raw_full_kelly_percentage = combined_parlay_ev / b_decimal_odds_minus_one
                    optimized_fractional_kelly = raw_full_kelly_percentage * kelly_fraction_scalar
                    
                    # Clamp risk profile metrics between a baseline floor of 1% and a max safety ceiling of 15%
                    final_clamped_stake_fraction = max(0.01, min(0.15, optimized_fractional_kelly))
                    suggested_rand_stake = int(round((active_pool * final_clamped_stake_fraction) / 10.0) * 10.0)
                    
                    st.success(f"🎯 **Kelly Analysis Complete:** Recommended Bankroll Allocation Fraction is **{final_clamped_stake_fraction * 100:.1f}%**")
                else:
                    suggested_rand_stake = 0
                    st.error("✋ PARLAY STAKING LOCKED OUT: Combined expected value is negative. Your model detects zero edge.")
                
                # Display potential returns panel
                potential_payout_return = float(suggested_rand_stake) * combined_bookmaker_odds
                net_profit_return = potential_payout_return - float(suggested_rand_stake)
                
                st.write(f"💰 **Estimated Ticket Return:** Recommended Stake: **R {suggested_rand_stake}** | Payout: **R {potential_payout_return:.2f}** | Net Profit: **R {net_profit_return:.2f}**")
                
                # 📑 STEP 3: DYNAMIC PARLAY TICKETING SLIP EXPORTER
                st.markdown("##### 📥 Finalize Export Protocol")
                
                legs_string_block = "\n".join(parlay_market_names)
                parlay_slip_text_payload = (
                    f"=========================================\n"
                    f"       SISONKE HUB MULTI-LEG TICKET      \n"
                    f"=========================================\n"
                    f"Matchday Reference: {home_target_key} vs {away_target_key}\n"
                    f"Timestamp Logged: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                    f"SELECTED PARLAY LEGS:\n"
                    f"{legs_string_block}\n\n"
                    f"COMBINED EFFICIENCY PROFILE:\n"
                    f"* Sportsbook Odds: {combined_bookmaker_odds:.2f}x\n"
                    f"* True Fair Odds: {combined_fair_odds:.2f}x\n"
                    f"* Combined Model Edge: {combined_parlay_ev * 100:+.1f}%\n"
                    f"* Applied Kelly Scale: {kelly_fraction_scalar:.2f}x\n\n"
                    f"STAKING CONFIGURATION RE-BALANCER:\n"
                    f"* Total Campaign Bankroll: R {active_pool:.2f}\n"
                    f"* Recommended Risk Fraction: {final_clamped_stake_fraction * 100:.1f}%\n"
                    f"* Target Bet Strike Stake: R {suggested_rand_stake}\n"
                    f"* Projected Ticket Return: R {potential_payout_return:.2f}\n"
                    f"=========================================\n"
                    f"          WE BEAT THE ODDS.              \n"
                    f"=========================================\n"
                )
                
                if suggested_rand_stake > 0:
                    st.download_button(
                        label="📥 Download Parlay Betting Slip (.TXT)", 
                        data=parlay_slip_text_payload, 
                        file_name=f"sisonke_parlay_slip_{home_target_key.lower()}.txt", 
                        mime="text/plain",
                        key="sisonke_parlay_download_button_v2026"
                    )
                else:
                    st.warning("⚠️ Betting Slip download disabled due to negative value edges.")
            else:
                st.info("💡 Add 2 or more betting lines using the selector box above to build your parlay stake configurations.")
        else:
            st.warning("⚠️ Generate a match projection grid first to feed rows into the parlay builder slot.")
    ...
    if not filtered_df.empty:
        ...
        if xpts_rows:
            ...
            st.warning("⚠️ No historical matrix records found for this league selection block.")
                ...
                # ==============================================================================
        # UPGRADED SEGMENT 13: 10,000 MC CHAMPS & RELEGATION RISK TRACKER COMBINED
        # ==============================================================================
        st.markdown("##### 🔮 10,000 Monte Carlo Outright Championship & Relegation Simulator")
        num_simulations_pass = 10000
        simulated_championship_tally = {t: 0 for t in all_teams_raw}
        simulated_relegation_tally = {t: 0 for t in all_teams_raw}  # Tracks bottom finishes
        ...
        for sim_run in range(num_simulations_pass):
            ...
            for t in all_teams_raw:
                t_prof_data = team_simulation_profiles.get(t, {"base_points": 0, "att_vector": 1.45})
                current_iter_standings[t] = t_prof_data.get("base_points", 0)
            ...
            for i, team_a in enumerate(all_teams_raw):
                for j, team_b in enumerate(all_teams_raw):
                    if i != j:
                        ...
                        lambda_a = float(profile_a.get("att_vector", 1.45)) * automatically_tuned_hfa_factor
                        lambda_b = float(profile_b.get("att_vector", 1.45))
                        
                        if np.random.poisson(lambda_a) > np.random.poisson(lambda_b): 
                            current_iter_standings[team_a] += 3
                        elif np.random.poisson(lambda_a) < np.random.poisson(lambda_b): 
                            current_iter_standings[team_b] += 3
                        else: 
                            current_iter_standings[team_a] += 1; current_iter_standings[team_b] += 1
                        
            for t in all_teams_raw:
                simulated_total_points_accumulator[t] += current_iter_standings[t]
                
            if current_iter_standings:
                # 🥇 Track the League Winner
                winner_squad = max(current_iter_standings, key=current_iter_standings.get)
                simulated_championship_tally[winner_squad] += 1
                
                # 🔻 Track Relegation (The squad finishing dead last in this run)
                loser_squad = min(current_iter_standings, key=current_iter_standings.get)
                simulated_relegation_tally[loser_squad] += 1

        st.markdown("---")
        st.header("🏆 Divisional Outright Mass Entry Deck")
        st.markdown("Type in the live outright title winner odds from Hollywoodbets or Easybet for all teams simultaneously.")
        ...
        st.markdown("##### 📊 Integrated Outright & Relegation Value Ledger")

        outright_rendered_payload = []
        for team in sorted(all_teams_raw):
            final_win_probability = simulated_championship_tally[team] / num_simulations_pass
            final_relegation_probability = simulated_relegation_tally[team] / num_simulations_pass  # Clean calculation
            clamped_prob = max(0.001, final_win_probability)
            fair_zero_margin_odds = 1.0 / clamped_prob
            ...
            # Determine dynamic security status indicators for styling rows
            if final_relegation_probability > 0.40:
                safety_alert_flag = "🚨 CRITICAL DROP RISK"
            elif 0.10 <= final_relegation_probability <= 0.40:
                safety_alert_flag = "⚠️ DANGER BOUNDS"
            else:
                safety_alert_flag = "🟢 MATHEMATICALLY SAFE"
            
            outright_rendered_payload.append({
                "Squad Contender Team": team,
                "Projected Avg Points": expected_end_points_average,
                "Championship Prob (%)": f"{final_win_probability * 100:.1f}%",
                "True Fair Title Odds": f"{fair_zero_margin_odds:.2f}",
                "Bookmaker Title Odds": f"{live_bookie_odds:.2f}",
                "Outright Title Edge (EV)": f"{outright_expected_value * 100:+.1f}%",
                "Simulated Relegation Risk": f"{final_relegation_probability * 100:.1f}%", # RELEGATION COLUMN ACTIVE
                "Divisional Safety Status": safety_alert_flag
            })
            
        st.dataframe(
            pd.DataFrame(outright_rendered_payload).sort_values(by="Projected Avg Points", ascending=False),
    ...
    if not filtered_df.empty and len(settled_past_games) >= 3:
        try:
            ...
            if b_df is not None and not b_df.empty:
                ...
                else: st.info("ðŸ“Š Validation Standby: Requirements deficit pricing lines.")
                
                with st.expander("ðŸ¦… Team Form Shift Diagnostic Monitor (Trend Graph)", expanded=True):
                ...
                with st.expander("ðŸ’° Team Historical Odds Performance & CLV Tracker", expanded=False):
...
with tab_past:
    st.markdown("### ðŸ“œ Settled Historical Results & Proxy xG vs Goal Difference Audit Table")
    if not filtered_df.empty:
